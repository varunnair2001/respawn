Breakout = {
  ...
  setLevel: function(level) {
    ...
    globalThis.localStorage['level'] = level;
    this.refreshDOM();
  },
  ...
  Score: {
    ...
    load:     function(game)  { var xhttp = new XMLHttpRequest();
                                var url = '/games/breakout';
                                xhttp.onreadystatechange = function() {
                                  if (this.readyState == 4 && this.status == 200) {
                                      // Typical action to be performed when the document is ready:
                                      var data = JSON.parse(xhttp.responseText);
                                      globalThis.localStorage.highscore = Number(data.highscore) ? parseInt(Number(data.highscore)) : 0;
                                  }
                                };
    ...
    save:     function()  {
      ...
      if (this.score > globalThis.localStorage.highscore){
        this.game.storage.highscore = this.score;
        globalThis.localStorage.highscore = this.score;
        console.log(globalThis.localStorage.highscore);
        var xhttp = new XMLHttpRequest();
        var url = `/games/breakout/${Number(globalThis.localStorage.level)+1}/${globalThis.localStorage.highscore}`;
        xhttp.open("POST", url, true);
        xhttp.send();                               
      }
      var xhttp = new XMLHttpRequest();
      var url = `/games/breakout/${Number(globalThis.localStorage.level)+1}/${this.score}/${startGameTime}/${timeDelta}`;
      xhttp.open("POST", url, true);
      xhttp.send();
      location.reload();
     },
    ...
    render: function(ctx) {
      ...
      text          = "HIGH SCORE: " + this.format(ishigh ? this.score : globalThis.localStorage.highscore);
      ctx.font      = this.highfont;
