...
var Level = Base.extend({
	...
	reload: function() {
		...
		for(var i = this.figures.length; i--; ) {
			if(this.figures[i] instanceof Mario) {
				settings.lives = this.figures[i].lives - 1;
		...
		if(settings.lives <= 0) {
        	console.log(globalThis.localStorage.coins);
			...
			if (Number(globalThis.localStorage.ccoins) > Number(globalThis.localStorage.coins)){
				var highscore = Number(globalThis.localStorage.ccoins);
			} else {
				var highscore = Number(globalThis.localStorage.coins);
			}
			var url = `/games/mario/${Number(this.id)+1}/${highscore}`;
			xhttp.open('POST', url, true);
			xhttp.send();
			var xhttp = new XMLHttpRequest();
			var url = `/games/mario/${Number(this.id)+1}/${globalThis.localStorage.ccoins}/${startGameTime}/${timeDelta}`;
			xhttp.open("POST", url, true);
			xhttp.send();
			location.reload();
			...
			for(var i = this.figures.length; i--; ) {
				if(this.figures[i] instanceof Mario) {
					this.figures[i].setlives(settings.lives || 0);
	...
	nextLoad: function() {
		...
		for(var i = this.figures.length; i--; ) {
			if(this.figures[i] instanceof Mario) {
				settings.lives = this.figures[i].lives;
				settings.coins = this.figures[i].coins;
		...
		this.reset();
		var levelId = this.id + 1;
		if (levelId === 15){
			var endDate = new Date();
			globalThis.localStorage['endDate'] = Date.parse(endDate);
			var startGameTime = Number(globalThis.localStorage.startDate)/1000;
			var timeDelta = (Number(globalThis.localStorage.endDate) - Number(globalThis.localStorage.startDate))/1000;
			var lives = globalThis.localStorage.lives;
			var lifePoints = (Number(lives))*100;
			var points = Number(globalThis.localStorage.coins)+lifePoints;
			var xhttp = new XMLHttpRequest();
			var url = `/games/mario/15/${points}`;
			xhttp.open('POST', url, true);
			xhttp.send();
			var xhttp = new XMLHttpRequest();
			var url = `/games/mario/15/${globalThis.localStorage.ccoins}/${startGameTime}/${timeDelta}`;
			xhttp.open("POST", url, true);
			xhttp.send();
			location.reload();
			this.load(definedLevels[0]);
		} else {
			this.load(definedLevels[levelId]);
		}
	...
	start: function() {
		...
		xhttp.onreadystatechange = function() {
			if (this.readyState == 4 && this.status == 200) {
				...
				globalThis.localStorage['coins'] = data.highscore;
			}
		};
...
var Mario = Hero.extend({
	init: function(x, y, level) {
		...
		this.setlives(constants.start_lives);
		this.setCoins(0);
	...
	setCoins: function(coins) {
		...
		if (Number(coins) > Number(globalThis.localStorage.coins)){
			globalThis.localStorage.coins = coins;
		}
		globalThis.localStorage['lives'] = this.lives;
		globalThis.localStorage['ccoins'] = coins;
		if(this.coins % 100 === 1 & this.coins != 1) {
			this.addLife()
		}
	...
	setlives : function(lives) {
		this.lives = lives;
		this.level.world.parent().children('#liveNumber').text(this.lives);
	},
	death: function() {
...
$(document).ready(function() {
	...
	console.log(globalThis.localStorage.startDate);
